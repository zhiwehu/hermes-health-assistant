...
### 🌅 白天（实时记录）
用户发送健康相关消息（文字/语音/图片） → 自动追加到 `conversations/YYYY-MM-DD.md`
- **不打扰用户**：默默记录，除非用户询问
- **结构化提取**：体重、体脂、心率、睡眠阶段、食物热量估算
- **图片处理**：保留原始缓存文件名，用相对路径引用
...
### 🌆 晚间（22:00 总结引导）
Cron Job 自动触发，发送今日总结草稿给用户：
- 汇总白天记录的所有数据
- 提示用户补充缺失项（体重/运动/睡眠/感受）
- 给出初步评分和建议

### 🌙 深夜（日记定稿）
用户确认/补充后，整合数据写入 `YYYY-MM-DD.md`（最终日记）
